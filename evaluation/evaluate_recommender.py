...
from evaluation.samplers.sampler import TargetItemSampler
from utils.os_utils import mkdir_p, shell
from evaluation.evaluation_utils import group_by_user
...
def evaluate_recommender(recommender, test_actions,
    ...
    user_docs = []
    for i in tqdm(range(len(requests)), ascii=True, bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}', position=0,
                  leave=True, ncols=70):
...
class RecommendersEvaluator(object):
    def __init__(self, actions, recommenders, metrics, out_dir, data_splitter,
        ...
        tb.configure(argv=[None, '--logdir', tensorboard_dir, '--host', '0.0.0.0', '--port', '6006'])
        url = tb.launch()
        ...
        if not isinstance(self.data_splitter, GlobalTemporalSplit):
            self.train, self.test = self.data_splitter(actions)
        else:
            self.train, self.test, self.val_border_timestamp = self.data_splitter(actions)
        ...
        if target_items_sampler is not None:
            ...
            print(
                f"sampled requests generated in {sampled_requests_generation_start - sampled_requests_generation_end} seconds")
    ...
    def log_result_to_tensorboard(self, evaluation_result):
        ...
        with torch.device("cpu"):
            for metric in self.metrics:
                ...
                tensorboard_writer.add_scalar(metric_name, metric_value, global_step=step)
        pass
    ...
    def evaluate_single_recommender(self, recommender_name, result_queue):
        try:
            ...
            os.symlink(os.path.abspath(tensorboard_dir),
                       os.path.abspath((self.global_tensorboard_dir / tensorboard_run_id)))
            ...
            for action in tqdm(self.train, ascii=True, bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}', position=0,
                               leave=True, ncols=70):
            ...
            if not isinstance(self.data_splitter, GlobalTemporalSplit):
                ...
                recommender.rebuild_model(self.val_border_timestamp)

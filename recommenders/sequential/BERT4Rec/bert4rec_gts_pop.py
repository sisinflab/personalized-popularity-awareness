...
class MaskingCollator(object):
    def __init__(self, user_actions, sequence_length, val_users,
                 masking_prob, pad_id, mask_id, ignore_val=-100, mode='train', border_timestamp=None, val_ndcg_at=None,
                 n_items=None) -> None:
    ...
    def __call__(self, batch):
        ...
        for user_id in batch:
            if self.is_train:
                ...
                else:  # the items after the border timestamp are removed (they are used for the evaluation)
                ...
                # always mask the last token that is used for train ndcg evaluation (the random masked position are all but one)
                ...
                if seq_after_ts.shape[0] != 0:
                    # Separate track IDs and ratings
                    ...
                    # Update ratings for likes and dislikes directly
                    ratings.index_put_((track_ids[likes_dislikes_mask],), track_ratings[likes_dislikes_mask])
                    # Get current ratings for tracks played or skipped
                    current_ratings = ratings[track_ids[plays_skips_mask]]
                    # Mask for tracks that are not already liked or disliked
                    update_mask = (current_ratings != -2) & (current_ratings != 2)
                    # Update ratings for plays and skips where applicable
                    ratings.index_put_((track_ids[plays_skips_mask][update_mask],),
    ...
    def rebuild_model(self, val_border_timestamp=None):
        ...
        for epoch in range(self.max_epochs):
            ...
            for step, batch in enumerate(train_loader):
                ...
                logits = self.bert.forward(input_ids=seq, attention_mask=attention_mask).logits
                popularity_logits = self.get_popularity_logits(seq)

                if step == batches_per_epoch - 1:
                    gt_logits = logits.detach().gather(-1, torch.relu(labels.unsqueeze(-1))).squeeze(-1).reshape(
                    ...
                    gt_pop_logits = popularity_logits.expand_as(logits).gather(-1, torch.relu(
                        labels.unsqueeze(-1))).squeeze(-1).reshape(seq.shape[0] * seq.shape[1])
                    mean_gt_pop_logits = (
                    ...
                    mask.scatter_(-1, torch.relu(labels.unsqueeze(-1)), False)
                    mask[:, :, self.items.size():] = False
                    not_gt_logits = logits.detach()[mask]
                    ...
                    del not_gt_logits
                    not_gt_pop_logits = popularity_logits.expand_as(logits)[mask]
                    ...
                    tb_writer.add_scalars("score/bert_contribution",
                ...
                new_logits[:, :, self.items.size():] = float('-inf')

                loss = self.loss_fct(new_logits.view(-1, self.bert_config.vocab_size),
    ...
    def recommend_impl(self, batch, limit, mode):
        with torch.no_grad():
            if mode == 'val':
                ...
                # self.bert.forward(input_ids=seq, attention_mask=attn).logits.data.shape -> [batch_size x sequence_length x vocab_size]
                logits = self.bert.forward(input_ids=seq, attention_mask=attn).logits + self.get_popularity_logits(seq)
                ...
                # self.bert.forward(input_ids=seq, attention_mask=attn).logits.data.shape -> [batch_size x sequence_length x vocab_size]
                logits = (self.bert.forward(input_ids=seq, attention_mask=attn).logits + self.get_popularity_logits(
                    seq))[:, -1, :]  # last one in the sequence

    ...
    def rebuild_model(self, val_border_timestamp=None):
        ...
        for epoch in range(self.max_epochs):
            ...
            for step, batch in enumerate(train_loader):
                ...
                loss_per_element = torch.nn.functional.binary_cross_entropy_with_logits(logits, gt,
                                                                                        reduction='none').mean(
                    -1) * mask
    ...
    def recommend_impl(self, batch, limit, mode):
        with torch.no_grad():
            if mode == 'val':
                ...
                logits = self.sasrec.get_logits(last_hidden_state)
                del last_hidden_state
                ...
                last_hidden_state, _ = self.sasrec.forward(input=seq)
                logits = self.sasrec.get_logits(last_hidden_state)[:, -1, :]
                del last_hidden_state

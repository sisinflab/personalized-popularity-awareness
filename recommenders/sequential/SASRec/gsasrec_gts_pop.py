...
class MaskingCollator(object):
    def __init__(self, user_actions, sequence_length, val_users, pad_id, mode='train', border_timestamp=None,
                 val_ndcg_at=None, n_items=None, n_negatives=None) -> None:
    ...
    def __call__(self, batch):
            ...
            if self.is_train:
                if user_id not in self.val_users:
                    seq = [x[1] for x in self.user_actions[user_id] if
                           x[2] > 0]  # sequence of the item id (x[1]) in the positive intractions of the user
        ...
        if self.is_test:
            ...
            negatives = torch.randint(low=0, high=self.pad_id,
            ...
            negatives = torch.randint(low=0, high=self.pad_id, size=(len(batch), self.n_negatives), requires_grad=False)
            batch = {"seq": torch.stack(seqs), "labels": torch.stack(labels_all), "negatives": negatives,
...
class gSASRecRecommender(Recommender):
    def __init__(self,
                 sequence_length=200,
                 embedding_size=256,
    ...
    def rebuild_model(self, val_border_timestamp=None):
        ...
        for epoch in range(self.max_epochs):
            ...
            for step, batch in enumerate(train_loader):
                ...
                alpha = self.negs_per_pos / (self.items.size() - 1)
                t = self.gbce_t
                self.beta = alpha * ((1 - 1 / alpha) * t + 1 / alpha)

                positive_logits = new_logits[:, :, 0:1].to(torch.float64)  #use float64 to increase numerical stability
                negative_logits = new_logits[:, :, 1:].to(torch.float64)
                eps = 1e-10
                positive_probs = torch.clamp(torch.sigmoid(positive_logits), eps, 1 - eps)
                positive_probs_adjusted = torch.clamp(positive_probs.pow(-self.beta), 1 + eps,
                ...
                new_logits = torch.cat([positive_logits_transformed, negative_logits], -1)

                mask = (seq[:, :-1] != self.pad_item_id).float()  # used to not compute loss for pad id item
                loss_per_element = torch.nn.functional.binary_cross_entropy_with_logits(new_logits, gt,
                                                                                        reduction='none').mean(
                    -1) * mask
    ...
    def recommend_impl(self, batch, limit, mode):
        with torch.no_grad():
            if mode == 'val':
                ...
                positive_logits = logits_for_loss[:, 0:1].to(
                    torch.float64)  #use float64 to increase numerical stability
